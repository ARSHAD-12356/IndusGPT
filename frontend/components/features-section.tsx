...
export function FeaturesSection() {
  return (
    <section id="features" className="relative py-28 bg-[#05050a] overflow-hidden">
      {/* Background glow */}
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {FEATURES.map((f) => (
            <div
              key={f.title}
              className={`group relative flex flex-col gap-5 p-6 rounded-2xl border border-white/8 bg-white/[0.03] hover:bg-white/[0.06] backdrop-blur-sm shadow-xl transition-all duration-300 cursor-default overflow-hidden ${f.glow} hover:shadow-2xl ${f.border}`}
            >
              {/* Glow spot */}
